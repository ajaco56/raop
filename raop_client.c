...
static int fd_event_callback(void *p, int flags)
{
	...
	if(flags&RAOP_FD_READ){
		i=(int)read(raopcld->sfd,buf,sizeof(buf));
		if(i>0){
	...
	i=(int)write(raopcld->sfd,raopcld->data+raopcld->wblk_wsize,raopcld->wblk_remsize);
	if(i<0){
...
int raopcl_sync(void *args)
{
	...
	while(1)
	{
        ...
        if ( isFirst )
            {
		    data[0] = (unsigned char)144;	//RTP protocol (0x80) + extension ( 0x10 )
            isFirst=0;
            }
        else
            {
		    data[0] = (unsigned char)128;	//RTP protocol ( 0x80 )
            }
		...
		data[1] = (unsigned char)212;  //payload type=84 + marker bit set = 212
		...
		i = (int)write(raopcld->cfd,&data,sizeof(data));
...
static int raopcl_connecttime(raopcl_data_t *raopcld, uint16_t myport)
{
	...
	char c = (unsigned char)255;
	int i = (int)write(raopcld->tfd,&c,1);
	if(i<0){
		ERRMSG("%s: write error: %s\n", __func__, strerror(errno));
		return -1;
	}
...
int raopcl_time_connect(void *args)
{
	...
	while(1)
	{
		i=(int)read(raopcld->tfd,buf,sizeof(buf));
		if(i>0)
		{
			// DBGMSG("%s: read %d bytes, rsize=%d\n", __func__, i,rsize);
			char data[32];  //geen SSRC
            data[0] = (unsigned char)128;	//RTP protocol
			...
			i = (int)write(raopcld->tfd,&data,sizeof(data));
                        if ( i != sizeof(data) )
                        {
...
int raopcl_connect(raopcl_t *p, char *host, uint16_t destport, int encrypt, int volume)
{
	...
	if(rtspcl_set_useragent(raopcld->rtspcl,"RAOP/CDust Creative Engineering (Macintosh Intel, Mac OS X 11.2.1)")) goto erexit;
	...
	if(realloc_memory((void**)&raopcld->addr,(int)strlen(host)+1,__func__)) goto erexit;
