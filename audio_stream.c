 ...
 *
 * You should have received a copy of the GNU General Public Licensegfdg
...
int auds_write_pcm(auds_t *auds, uint8_t *buffer, uint8_t **data, int *size, int bsize, data_source_t *ds)
{
    //INFMSG("%s: size: %d, bsize: %d buffer: %s data: %d sample_rate: %d channels: %d chunk-size: %d type: %d fd: %d\n",
    //__FUNCTION__,*size,bsize, buffer, **data, auds->sample_rate,auds->channels,auds->chunk_size, ds->type,ds->u.fd);//,ds->u.fd);
    ...
    while(1){
        ...
        //DBGMSG ("Count: %03d %02x %02x %02x %02x \n", count, one[0],one[1],one[2],one[3]);
...
static data_type_t get_data_type(char *fname)
{
    ...
    if(strstr(fname,"http")==fname) return AUD_TYPE_MP3;
