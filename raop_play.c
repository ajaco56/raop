...
int main(int argc, char *argv[])
{
    ...
    char *r = (char*) &value;
    if (*r == 1)
        INFMSG("Little Endian\n");
    else INFMSG("Big Endian\n");
    
//    uint32_t in = 0x12345678;
//    DBGMSG( "in: %08x \n", in);
//    uint8_t out[4];
//    *(uint32_t*)&out = in;
//    DBGMSG( "outle: %08x %08x %08x %08x \n", out[0],out[1],out[2],out[3]);
//    //DBGMSG( "outbe: %08x %08x %08x %08x \n", out[0]&0xff>>24,out[1]&0xff>>16,out[2]&0xff>>8,out[3]);
//
//    uint8_t in2[4] = {0x78, 0x56, 0x34, 0x12};
//    DBGMSG( "in: %08x %08x %08x %08x \n", in2[0],in2[1],in2[2],in2[3]);
//    uint32_t out2;
//    out2 = *(uint32_t*)&in2;
//    DBGMSG( "outbe: %08x  \n", out2);
    
    uint16_t v = 0x0001234;
    uint8_t a[4];
    ...
    a[2] = vp[2];
    a[3] = vp[3];
    DBGMSG( "outbe: %04x%04x %04x%04x \n", a[0],a[1],a[2],a[3]);
    
    a[0] = v >> 24;
    a[1] = v >> 16;
    a[2] = v >>  8;
    a[3] = v;
    DBGMSG( "outle: %04x%04x %04x%04x \n", a[0],a[1],a[2],a[3]);
