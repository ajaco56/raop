...
int get_tcp_connect(int sd, struct sockaddr_in dest_addr)
{
	...
	if(connect(sd, (struct sockaddr *)&dest_addr, sizeof(struct sockaddr))){
		...
		// try one more time
		if(connect(sd, (struct sockaddr *)&dest_addr, sizeof(struct sockaddr))){
			ERRMSG("error:get_tcp_connect addr=%s, port=%d\n",
...
int bind_host(int sd, char *hostname, unsigned long ulAddr,unsigned short *port)
{
	...
	if(hostname){
		...
		// if hostname=NULL, use INADDR_ANY
		if(ulAddr)
			my_addr.sin_addr.s_addr = (int)ulAddr;
		else
...
int child_start(char* const argv[], int *infd, int *outfd, int *errfd)
{
	...
	if(pid==0){
        ...
        INFMSG("%s: CHILD, command: %s %s %s pid: %d\n",__FUNCTION__,argv[0], argv[1],argv[2],pid);
        ...
        if (strcasecmp(argv[0],"mpg123")){//mp3 stream
            //INFMSG("%s: STARTED, command: %s %s %s pid: %d\n",__FUNCTION__,argv[0], argv[1],argv[2],pid);
        }
        else INFMSG("%s: STARTED, command: %s %s %s %s %s %s %s pid: %d\n",__FUNCTION__,argv[0], argv[1],argv[2], argv[3],argv[4],argv[5],argv[6],pid);//flac stream
    ...
    INFMSG("%s: PARENT, command: %s %s %s pid: %d\n",__FUNCTION__,argv[0], argv[1],argv[2],pid);
    ...
    //INFMSG("%s: START, command: %s %s %s pid: %d\n",__FUNCTION__,argv[0], argv[1],argv[2],pid);
