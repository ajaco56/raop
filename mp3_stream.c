...
#include <sys/wait.h>
#include <fcntl.h>
#include "/usr/local/include/id3tag.h"
...
#define MP3_DECODER "/usr/local/bin/mpg123"
//#define MP3_DECODER "/usr/local/bin/mpg321"
//#define MP3_DECODER "/usr/local/bin/mpv"
static int run_decoder(auds_t *auds, mp3_t *mp3)
{
	INFMSG("entering decoder\n");
    
    char *darg[4]={MP3_DECODER,"-s",NULL, NULL};
    //char *darg[4]={MP3_DECODER, "",NULL,NULL};
	//int efd;
	//char buf[1024];
	//char *bufp;
	//int rsize,tsize=0,i;
    ...
    INFMSG("%s: audiotype: %u\n",__FUNCTION__,auds->data_type);
    
	if(auds->data_type==AUD_TYPE_MP3){
        INFMSG("%s: filename: %s\n",__FUNCTION__,mp3->fname);
    ...
    while(1){
		...
		rsize=(int)read(efd,buf+tsize,1023-tsize);
...
static int read_header(auds_t *auds, char *fname)
{
    ...
    if (id3file) {
		...
		INFMSG("%s: id3_file_open can't open file, go ahead anyway\n",__FUNCTION__);
	}
    ...
    INFMSG("wtf2\n");
    INFMSG("go with default sample rate\n");
...
int mp3_get_next_sample(auds_t *auds, __u8 **data, int *size)
{
    ...
    //data_source_t ds={.type=STREAM, .u.fd=mp3->dfd};
    
    //INFMSG("%s: data: %d, size: %d, chunk-size: %d, dastasource: %d\n",__FUNCTION__,**data,*size,auds->chunk_size,ds.u.fd);
    
    //INFO: data: 32, size: 1416 chunk-size: 352
