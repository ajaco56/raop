...
int rtspcl_add_exthds(rtspcl_t *p, char *key, char *data)
{
    ...
    if(realloc_memory((void*)&rtspcld->exthds[i].key,(int)strlen(key),__func__)) return -1;
    strcpy((char*)rtspcld->exthds[i].key,key);
    if(realloc_memory((void*)&rtspcld->exthds[i].data,(int)strlen(data),__func__)) return -1;
...
int rtspcl_setup(rtspcl_t *p, key_data_t **kd)
{
    ...
    if(realloc_memory((void*)&buf,(int)strlen(rtspcld->transport)+1,__func__)) goto erexit;
...
int rtspcl_auth_setup(rtspcl_t *p)
{
    ...
    */
    ...
    /*char data[] = {
    0x59, 0x02, 0xed, 0xe9, 0x0d, 0x4e, 0xf2, 0xbd, 0x4c, 0xb6, 0x8a,
    0x63, 0x30, 0x03, 0x82, 0x07, 0xa9, 0x4d, 0xbd, 0x50, 0xd8, 0xaa,
    0x46, 0x5b, 0x5d, 0x8c, 0x01, 0x2a, 0x0c, 0x7e, 0x1d, 0x4e
    };*/
                ...
                char *content, int length, int get_response, key_data_t *hds, key_data_t **kd, char* url)
{
    ...
    const char delimiters[] = " ";
    char *token,*dp;
    int i,j,dsize = 0,rval,totallength = 0;
    ...
    while( hds && hds[i].key != NULL )
    {
        ...
        strncat(req,reql,sizeof(req) - strlen(req) - 1);
        i++;
    }
    ...
    if( content_type && content)
    {
        ...
        {
            totallength = (int)strlen(req) + length;
    ...
    if(!length)
    {
        rval=(int)write(rtspcld->fd,req,strlen(req));
        DBGMSG("----> %s : write %s\n",__func__, req);
        if ( rval != strlen(req) )
        {
            ERRMSG("couldn't write request (%d!=%lu)\n",rval,strlen(req));
        }
    }
    else
    {
        rval=(int)write(rtspcld->fd,req,totallength);
        DBGMSG("----> %s : write %s\n",__func__,req);
        if ( rval != totallength )
        {
    ...
    if(token==NULL || strcmp(token,"200"))
    {
        if(get_response==1)
        {
            ERRMSG("<------ : %s: request failed, error %s %s\n",__func__,line,token);
            return -1;
        }
    }
    else
